        ...
        <section className={`z-index-5 pageSetup header-container ${isScrolling ? `position-fixed top-go-up-6rem` : 'position-fixed top-0'}`}>
            <div className={`header-top-bg ${isScrolling ? `` : ''}`}>
                <div className={`container header-top-contain `}>
                    <div className="logo"><Link to='/react-film-route' className={`${isActive => isActive ? '' : ''}`}><img src={pnglogo} alt='pnglogo' /></Link></div>
                    <Search isScrolling={isScrolling}/>
                    <div className="user-icon">
                        <NavLink to='/react-film-route/user' className={`fa-regular fa-user ${isActive => isActive ? 'active' : ''}`}></NavLink>
                        <NavLink to='/react-film-route/favourite' className={`fa-regular fa-bookmark ${isActive => isActive ? 'active' : ''}`}></NavLink>
                    </div>
                </div>
            </div>
